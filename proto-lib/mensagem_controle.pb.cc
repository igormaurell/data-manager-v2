// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: mensagem_controle.proto

#define INTERNAL_SUPPRESS_PROTOBUF_FIELD_DEPRECATION
#include "mensagem_controle.pb.h"

#include <algorithm>

#include <google/protobuf/stubs/common.h>
#include <google/protobuf/stubs/once.h>
#include <google/protobuf/io/coded_stream.h>
#include <google/protobuf/wire_format_lite_inl.h>
#include <google/protobuf/descriptor.h>
#include <google/protobuf/generated_message_reflection.h>
#include <google/protobuf/reflection_ops.h>
#include <google/protobuf/wire_format.h>
// @@protoc_insertion_point(includes)

namespace {

const ::google::protobuf::Descriptor* PacoteControle_descriptor_ = NULL;
const ::google::protobuf::internal::GeneratedMessageReflection*
  PacoteControle_reflection_ = NULL;
const ::google::protobuf::EnumDescriptor* PacoteControle_LadoCampo_descriptor_ = NULL;
const ::google::protobuf::EnumDescriptor* PacoteControle_CorEquipe_descriptor_ = NULL;

}  // namespace


void protobuf_AssignDesc_mensagem_5fcontrole_2eproto() {
  protobuf_AddDesc_mensagem_5fcontrole_2eproto();
  const ::google::protobuf::FileDescriptor* file =
    ::google::protobuf::DescriptorPool::generated_pool()->FindFileByName(
      "mensagem_controle.proto");
  GOOGLE_CHECK(file != NULL);
  PacoteControle_descriptor_ = file->message_type(0);
  static const int PacoteControle_offsets_[3] = {
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(PacoteControle, ladocampo_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(PacoteControle, corequipe_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(PacoteControle, idgoleiro_),
  };
  PacoteControle_reflection_ =
    new ::google::protobuf::internal::GeneratedMessageReflection(
      PacoteControle_descriptor_,
      PacoteControle::default_instance_,
      PacoteControle_offsets_,
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(PacoteControle, _has_bits_[0]),
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(PacoteControle, _unknown_fields_),
      -1,
      ::google::protobuf::DescriptorPool::generated_pool(),
      ::google::protobuf::MessageFactory::generated_factory(),
      sizeof(PacoteControle));
  PacoteControle_LadoCampo_descriptor_ = PacoteControle_descriptor_->enum_type(0);
  PacoteControle_CorEquipe_descriptor_ = PacoteControle_descriptor_->enum_type(1);
}

namespace {

GOOGLE_PROTOBUF_DECLARE_ONCE(protobuf_AssignDescriptors_once_);
inline void protobuf_AssignDescriptorsOnce() {
  ::google::protobuf::GoogleOnceInit(&protobuf_AssignDescriptors_once_,
                 &protobuf_AssignDesc_mensagem_5fcontrole_2eproto);
}

void protobuf_RegisterTypes(const ::std::string&) {
  protobuf_AssignDescriptorsOnce();
  ::google::protobuf::MessageFactory::InternalRegisterGeneratedMessage(
    PacoteControle_descriptor_, &PacoteControle::default_instance());
}

}  // namespace

void protobuf_ShutdownFile_mensagem_5fcontrole_2eproto() {
  delete PacoteControle::default_instance_;
  delete PacoteControle_reflection_;
}

void protobuf_AddDesc_mensagem_5fcontrole_2eproto() {
  static bool already_here = false;
  if (already_here) return;
  already_here = true;
  GOOGLE_PROTOBUF_VERIFY_VERSION;

  ::google::protobuf::DescriptorPool::InternalAddGeneratedFile(
    "\n\027mensagem_controle.proto\"\313\001\n\016PacoteCont"
    "role\022,\n\tladoCampo\030\001 \002(\0162\031.PacoteControle"
    ".LadoCampo\022,\n\tcorEquipe\030\002 \002(\0162\031.PacoteCo"
    "ntrole.CorEquipe\022\021\n\tidGoleiro\030\003 \002(\005\"&\n\tL"
    "adoCampo\022\014\n\010ESQUERDO\020\000\022\013\n\007DIREITO\020\001\"\"\n\tC"
    "orEquipe\022\013\n\007AMARELO\020\000\022\010\n\004AZUL\020\001", 231);
  ::google::protobuf::MessageFactory::InternalRegisterGeneratedFile(
    "mensagem_controle.proto", &protobuf_RegisterTypes);
  PacoteControle::default_instance_ = new PacoteControle();
  PacoteControle::default_instance_->InitAsDefaultInstance();
  ::google::protobuf::internal::OnShutdown(&protobuf_ShutdownFile_mensagem_5fcontrole_2eproto);
}

// Force AddDescriptors() to be called at static initialization time.
struct StaticDescriptorInitializer_mensagem_5fcontrole_2eproto {
  StaticDescriptorInitializer_mensagem_5fcontrole_2eproto() {
    protobuf_AddDesc_mensagem_5fcontrole_2eproto();
  }
} static_descriptor_initializer_mensagem_5fcontrole_2eproto_;

// ===================================================================

const ::google::protobuf::EnumDescriptor* PacoteControle_LadoCampo_descriptor() {
  protobuf_AssignDescriptorsOnce();
  return PacoteControle_LadoCampo_descriptor_;
}
bool PacoteControle_LadoCampo_IsValid(int value) {
  switch(value) {
    case 0:
    case 1:
      return true;
    default:
      return false;
  }
}

#ifndef _MSC_VER
const PacoteControle_LadoCampo PacoteControle::ESQUERDO;
const PacoteControle_LadoCampo PacoteControle::DIREITO;
const PacoteControle_LadoCampo PacoteControle::LadoCampo_MIN;
const PacoteControle_LadoCampo PacoteControle::LadoCampo_MAX;
const int PacoteControle::LadoCampo_ARRAYSIZE;
#endif  // _MSC_VER
const ::google::protobuf::EnumDescriptor* PacoteControle_CorEquipe_descriptor() {
  protobuf_AssignDescriptorsOnce();
  return PacoteControle_CorEquipe_descriptor_;
}
bool PacoteControle_CorEquipe_IsValid(int value) {
  switch(value) {
    case 0:
    case 1:
      return true;
    default:
      return false;
  }
}

#ifndef _MSC_VER
const PacoteControle_CorEquipe PacoteControle::AMARELO;
const PacoteControle_CorEquipe PacoteControle::AZUL;
const PacoteControle_CorEquipe PacoteControle::CorEquipe_MIN;
const PacoteControle_CorEquipe PacoteControle::CorEquipe_MAX;
const int PacoteControle::CorEquipe_ARRAYSIZE;
#endif  // _MSC_VER
#ifndef _MSC_VER
const int PacoteControle::kLadoCampoFieldNumber;
const int PacoteControle::kCorEquipeFieldNumber;
const int PacoteControle::kIdGoleiroFieldNumber;
#endif  // !_MSC_VER

PacoteControle::PacoteControle()
  : ::google::protobuf::Message() {
  SharedCtor();
  // @@protoc_insertion_point(constructor:PacoteControle)
}

void PacoteControle::InitAsDefaultInstance() {
}

PacoteControle::PacoteControle(const PacoteControle& from)
  : ::google::protobuf::Message() {
  SharedCtor();
  MergeFrom(from);
  // @@protoc_insertion_point(copy_constructor:PacoteControle)
}

void PacoteControle::SharedCtor() {
  _cached_size_ = 0;
  ladocampo_ = 0;
  corequipe_ = 0;
  idgoleiro_ = 0;
  ::memset(_has_bits_, 0, sizeof(_has_bits_));
}

PacoteControle::~PacoteControle() {
  // @@protoc_insertion_point(destructor:PacoteControle)
  SharedDtor();
}

void PacoteControle::SharedDtor() {
  if (this != default_instance_) {
  }
}

void PacoteControle::SetCachedSize(int size) const {
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
}
const ::google::protobuf::Descriptor* PacoteControle::descriptor() {
  protobuf_AssignDescriptorsOnce();
  return PacoteControle_descriptor_;
}

const PacoteControle& PacoteControle::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_mensagem_5fcontrole_2eproto();
  return *default_instance_;
}

PacoteControle* PacoteControle::default_instance_ = NULL;

PacoteControle* PacoteControle::New() const {
  return new PacoteControle;
}

void PacoteControle::Clear() {
#define OFFSET_OF_FIELD_(f) (reinterpret_cast<char*>(      \
  &reinterpret_cast<PacoteControle*>(16)->f) - \
   reinterpret_cast<char*>(16))

#define ZR_(first, last) do {                              \
    size_t f = OFFSET_OF_FIELD_(first);                    \
    size_t n = OFFSET_OF_FIELD_(last) - f + sizeof(last);  \
    ::memset(&first, 0, n);                                \
  } while (0)

  ZR_(ladocampo_, idgoleiro_);

#undef OFFSET_OF_FIELD_
#undef ZR_

  ::memset(_has_bits_, 0, sizeof(_has_bits_));
  mutable_unknown_fields()->Clear();
}

bool PacoteControle::MergePartialFromCodedStream(
    ::google::protobuf::io::CodedInputStream* input) {
#define DO_(EXPRESSION) if (!(EXPRESSION)) goto failure
  ::google::protobuf::uint32 tag;
  // @@protoc_insertion_point(parse_start:PacoteControle)
  for (;;) {
    ::std::pair< ::google::protobuf::uint32, bool> p = input->ReadTagWithCutoff(127);
    tag = p.first;
    if (!p.second) goto handle_unusual;
    switch (::google::protobuf::internal::WireFormatLite::GetTagFieldNumber(tag)) {
      // required .PacoteControle.LadoCampo ladoCampo = 1;
      case 1: {
        if (tag == 8) {
          int value;
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   int, ::google::protobuf::internal::WireFormatLite::TYPE_ENUM>(
                 input, &value)));
          if (::PacoteControle_LadoCampo_IsValid(value)) {
            set_ladocampo(static_cast< ::PacoteControle_LadoCampo >(value));
          } else {
            mutable_unknown_fields()->AddVarint(1, value);
          }
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(16)) goto parse_corEquipe;
        break;
      }

      // required .PacoteControle.CorEquipe corEquipe = 2;
      case 2: {
        if (tag == 16) {
         parse_corEquipe:
          int value;
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   int, ::google::protobuf::internal::WireFormatLite::TYPE_ENUM>(
                 input, &value)));
          if (::PacoteControle_CorEquipe_IsValid(value)) {
            set_corequipe(static_cast< ::PacoteControle_CorEquipe >(value));
          } else {
            mutable_unknown_fields()->AddVarint(2, value);
          }
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(24)) goto parse_idGoleiro;
        break;
      }

      // required int32 idGoleiro = 3;
      case 3: {
        if (tag == 24) {
         parse_idGoleiro:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int32, ::google::protobuf::internal::WireFormatLite::TYPE_INT32>(
                 input, &idgoleiro_)));
          set_has_idgoleiro();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectAtEnd()) goto success;
        break;
      }

      default: {
      handle_unusual:
        if (tag == 0 ||
            ::google::protobuf::internal::WireFormatLite::GetTagWireType(tag) ==
            ::google::protobuf::internal::WireFormatLite::WIRETYPE_END_GROUP) {
          goto success;
        }
        DO_(::google::protobuf::internal::WireFormat::SkipField(
              input, tag, mutable_unknown_fields()));
        break;
      }
    }
  }
success:
  // @@protoc_insertion_point(parse_success:PacoteControle)
  return true;
failure:
  // @@protoc_insertion_point(parse_failure:PacoteControle)
  return false;
#undef DO_
}

void PacoteControle::SerializeWithCachedSizes(
    ::google::protobuf::io::CodedOutputStream* output) const {
  // @@protoc_insertion_point(serialize_start:PacoteControle)
  // required .PacoteControle.LadoCampo ladoCampo = 1;
  if (has_ladocampo()) {
    ::google::protobuf::internal::WireFormatLite::WriteEnum(
      1, this->ladocampo(), output);
  }

  // required .PacoteControle.CorEquipe corEquipe = 2;
  if (has_corequipe()) {
    ::google::protobuf::internal::WireFormatLite::WriteEnum(
      2, this->corequipe(), output);
  }

  // required int32 idGoleiro = 3;
  if (has_idgoleiro()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt32(3, this->idgoleiro(), output);
  }

  if (!unknown_fields().empty()) {
    ::google::protobuf::internal::WireFormat::SerializeUnknownFields(
        unknown_fields(), output);
  }
  // @@protoc_insertion_point(serialize_end:PacoteControle)
}

::google::protobuf::uint8* PacoteControle::SerializeWithCachedSizesToArray(
    ::google::protobuf::uint8* target) const {
  // @@protoc_insertion_point(serialize_to_array_start:PacoteControle)
  // required .PacoteControle.LadoCampo ladoCampo = 1;
  if (has_ladocampo()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteEnumToArray(
      1, this->ladocampo(), target);
  }

  // required .PacoteControle.CorEquipe corEquipe = 2;
  if (has_corequipe()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteEnumToArray(
      2, this->corequipe(), target);
  }

  // required int32 idGoleiro = 3;
  if (has_idgoleiro()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt32ToArray(3, this->idgoleiro(), target);
  }

  if (!unknown_fields().empty()) {
    target = ::google::protobuf::internal::WireFormat::SerializeUnknownFieldsToArray(
        unknown_fields(), target);
  }
  // @@protoc_insertion_point(serialize_to_array_end:PacoteControle)
  return target;
}

int PacoteControle::ByteSize() const {
  int total_size = 0;

  if (_has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    // required .PacoteControle.LadoCampo ladoCampo = 1;
    if (has_ladocampo()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::EnumSize(this->ladocampo());
    }

    // required .PacoteControle.CorEquipe corEquipe = 2;
    if (has_corequipe()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::EnumSize(this->corequipe());
    }

    // required int32 idGoleiro = 3;
    if (has_idgoleiro()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int32Size(
          this->idgoleiro());
    }

  }
  if (!unknown_fields().empty()) {
    total_size +=
      ::google::protobuf::internal::WireFormat::ComputeUnknownFieldsSize(
        unknown_fields());
  }
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = total_size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
  return total_size;
}

void PacoteControle::MergeFrom(const ::google::protobuf::Message& from) {
  GOOGLE_CHECK_NE(&from, this);
  const PacoteControle* source =
    ::google::protobuf::internal::dynamic_cast_if_available<const PacoteControle*>(
      &from);
  if (source == NULL) {
    ::google::protobuf::internal::ReflectionOps::Merge(from, this);
  } else {
    MergeFrom(*source);
  }
}

void PacoteControle::MergeFrom(const PacoteControle& from) {
  GOOGLE_CHECK_NE(&from, this);
  if (from._has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    if (from.has_ladocampo()) {
      set_ladocampo(from.ladocampo());
    }
    if (from.has_corequipe()) {
      set_corequipe(from.corequipe());
    }
    if (from.has_idgoleiro()) {
      set_idgoleiro(from.idgoleiro());
    }
  }
  mutable_unknown_fields()->MergeFrom(from.unknown_fields());
}

void PacoteControle::CopyFrom(const ::google::protobuf::Message& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

void PacoteControle::CopyFrom(const PacoteControle& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

bool PacoteControle::IsInitialized() const {
  if ((_has_bits_[0] & 0x00000007) != 0x00000007) return false;

  return true;
}

void PacoteControle::Swap(PacoteControle* other) {
  if (other != this) {
    std::swap(ladocampo_, other->ladocampo_);
    std::swap(corequipe_, other->corequipe_);
    std::swap(idgoleiro_, other->idgoleiro_);
    std::swap(_has_bits_[0], other->_has_bits_[0]);
    _unknown_fields_.Swap(&other->_unknown_fields_);
    std::swap(_cached_size_, other->_cached_size_);
  }
}

::google::protobuf::Metadata PacoteControle::GetMetadata() const {
  protobuf_AssignDescriptorsOnce();
  ::google::protobuf::Metadata metadata;
  metadata.descriptor = PacoteControle_descriptor_;
  metadata.reflection = PacoteControle_reflection_;
  return metadata;
}


// @@protoc_insertion_point(namespace_scope)

// @@protoc_insertion_point(global_scope)
